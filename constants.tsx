...
export const Icons = {
  ...
  Zap: () => <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>,
  ...
  Stop: () => <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z M9 10a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1v-4z" /></svg>
};

export const getSystemInstruction = (config: SessionConfig): string => {
  const targetLang = config.userProfile?.targetLanguage || "English";
  const accent = config.accentPreference || "Standard";

  return `SYSTEM SETTING: VOICE_MODULATION_ACTIVE
  ACCENT_PROFILE: ${accent}
  TARGET_LANGUAGE: ${targetLang}
  
  You are an advanced AI Tutor (LinguaSphere Dean). Your voice output MUST reflect the requested accent: ${accent}.
  
  CORE DIRECTIVE: 
  You MUST speak with a heavy, distinct ${accent} accent at all times.
  Do not revert to a default/neutral AI voice. The user wants to hear ${accent}.
  
  When speaking English, you must overlay the phonetic characteristics of ${accent} (e.g. vowel shifts, rhoticity/non-rhoticity, intonation patterns typical of ${accent} speakers).
  If the target language is ${targetLang}, speak it with native fluency.

  Lesson Context: ${config.topic}
  Phase: ${config.phase.toUpperCase()}
  
  PHASE BEHAVIOR:
  - INSTRUCTION: Explain concepts concisely using the ${accent} accent.
  - PRACTICE: Drill the student. Correct their pronunciation deviations immediately.
  - EVALUATION: Act as a strict examiner with a strong ${accent} accent.

  Keep your responses concise and spoken naturally. Embody the persona of a strict but encouraging dialect coach from that region.
  `;
};
